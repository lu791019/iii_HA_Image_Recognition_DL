...
UPLOAD_PATH = 'C:/Users/Big data/Desktop/iii_HA/CT/DP_model_faster_RCNN/CT_img/'
        ...
        while True:
            ...
            for record in records:
                ...
                if record.error():
                    ...
                    #以os套件的path.exists()來確認dirname的位置是否存在,以not判別
                    if not os.path.exists(UPLOAD_PATH):
                        #os.mkdir()/os.makedirs()兩種方式 : mkdir只建最後一層的資料夾,makedirs則可以建立多層,故多年分要用makedirs
                        os.makedirs(UPLOAD_PATH)
                    with open(UPLOAD_FOLDER + str(msgKey)+'.png','wb') as w:
                        w.write(msgValue)
                    # 秀出metadata與 msgValue訊息

                    print('%s-%d-%d : (%s ,%s)\n' % (topic, partition, offset, timestamp, msgKey))
